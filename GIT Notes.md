GIT -Version control software -like (alpha version, Beta version, Gamma Version) **code version** 

**Father - Linus Torvalds**

-------------------------------------

1\) **Repositories**( Contains all folder related to this project)

2\) **Readme**( Basic Instruction for User)

3\) **Commit**( Save the Changes)





**GITHUB -** GitHub is web based hosting service software tool - Owned by Microsoft





Create Account in GITHUB



Step 1- https://github.com

step 2- Username : AjeethkumarKarthik , Password : Powndivya@143 Email id: vijayakarthikofficial@gmail.com

